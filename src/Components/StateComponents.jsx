        ...
        const fetchData = async () => {
            ...
            if(groupingState === 'Priority'){
                const filteredData = priorityArray.map((curStatus) => {
                    ...
                    const res = {name: curStatus.name, childData: data, len: data.length};
                    return res;
                })
  ...
  return (
        <div className='columnContainer'>
            {globalData.map((curStatus) => {
                ...
                return (
                    <div>
                        <HeaderCard title={curStatus.name} count={curStatus.len}/>
                    {curStatus.childData.map(data => {
                        return (
                            <Card id={data.id} title={data.title} tag={data.tag[0]} user={userData.filter((curData) => {
